...
export default function Feed() {
  ...
  async function loadFeedPage(offset: number, initial = false) {
    if (!initial && loadingMore) return
    setLoadingMore(true)
    try {
      const result = await api.getFeed(offset, PAGE_SIZE)
  ...
  const loadMore = useCallback(() => {
    ...
    loadFeedPage(offsetRef.current)
  }, [hasMore, loadingMore])
    ...
    <div className="page">
      ...
      <div style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '4px' }}>
          <h1 className="animate-fade-in-up" style={{ fontSize: '22px', fontWeight: 800 }}>Community</h1>
          {tab === 'feed' && (
      ...
      {tab === 'feed' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {feed.length === 0 && !loadingMore
            ? <Empty icon={<Radio size={32} strokeWidth={1} />} text="No live predictions" sub="Be the first to lock a prediction" />
          ...
          {!hasMore && feed.length > 0 && (
            <div style={{ textAlign: 'center', padding: '20px', fontSize: '12px', color: 'var(--text-muted)' }}>
              You've seen all live predictions
            </div>
          )}
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {leaderboard.length === 0
            ...
            : leaderboard.map((user, i) => (
              <div key={user.id} className="glass-glow animate-fade-in-up" onClick={() => navigate(`/u/${user.username}`)} style={{
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontWeight: 800, color: 'var(--accent-light)', fontSize: '16px' }}>{user.credit_score.toFixed(1)}</div>
          ...
          {trending.length === 0
            ...
            : trending.map((user, i) => (
              <div key={user.id} className="glass-glow animate-fade-in-up" onClick={() => navigate(`/u/${user.username}`)} style={{
                ...
                borderColor: i === 0 ? 'rgba(249,115,22,0.4)' : 'var(--border)', cursor: 'pointer',
              }}>
                <div style={{
                  ...
                  animation: i === 0 ? 'streak-burn 1.5s ease-in-out infinite' : 'none', flexShrink: 0,
                }}>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontWeight: 800, color: 'var(--accent-light)', fontSize: '16px' }}>{user.visibility_score.toFixed(1)}</div>
